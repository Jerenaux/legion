...
export abstract class Game {
  ...
  reconnectPlayer(socket: Socket) {
    ...
    const team = Array.from(this.teams.values()).find(
      (candidate) => candidate.teamData.playerUID === (socket as any).uid,
    );
  ...
  getPosition(index, flip, characterClass: Class) {
      ...
      for (let y = 0; y < GRID_HEIGHT; y++) {
        if (this.isFree(x, y) && !isSkip(x, y)) {
          ...
          if (characterClass === Class.WARRIOR) {
            if (x >= oneThirdWidth && x < twoThirdsWidth) {
              // Middle third of the battlefield
              availablePositions.push({ x, y });
            }
          } else if (characterClass === Class.BLACK_MAGE || characterClass === Class.WHITE_MAGE) {
            if (x < oneThirdWidth || x >= twoThirdsWidth) {
              // Outer thirds of the battlefield
              availablePositions.push({ x, y });
            }
          } else {
            // No restriction for other classes
            availablePositions.push({ x, y });
          }
        }
      }
    ...
    if (availablePositions.length === 0) {
      console.warn(
        `[Game:getPosition] No available position found for class ${characterClass}. Falling back to any valid position.`,
      );
      for (const x of xRange) {
        for (let y = 0; y < GRID_HEIGHT; y++) {
          if (this.isFree(x, y) && !isSkip(x, y)) {
  ...
  hasObstacle(gridX: number, gridY: number) {
    return this.terrainManager.terrainMap.get(`${gridX},${gridY}`) === Terrain.ICE || this.isHole(gridX, gridY);
  }
  ...
  processAttack({ target, sameTeam }: { target: number; sameTeam: boolean }) {
    ...
    if (!player.canAct() || !opponent.isAlive()) {
      return;
    }
  ...
  processObstacleAttack({ x, y }: { x: number; y: number }) {
    ...
    if (!player.canAct() || !player.isNextTo(x, y) || !this.hasObstacle(x, y)) return;
  ...
  applyMagic(spell: Spell, player: ServerPlayer, x: number, y: number, team: Team, targetPlayer: ServerPlayer | null) {
    ...
    if (spell.status) {
      targets.forEach((target) => {
        ...
        console.log(
          `[Game:applyMagic] Status effect ${spell.status.effect} applied to target ${target.num}: ${success}`,
        );
        if (success) {
  ...
  broadcastGEN(GENs: GEN[]) {
    ...
    GENs = GENs.filter(
      (gen) =>
        !this.GENhistory.has(gen) ||
        !noRepeatGENs.includes(gen) ||
        (noRepeatGENs.includes(gen) && !this.GENhistory.has(gen)),
    );
  ...
  listCellsInRange(gridX: number, gridY: number, radius: number): Tile[] {
    ...
    return hexTiles
      .filter((tile) => !isSkip(tile.x, tile.y) && this.isValidCell(gridX, gridY, tile.x, tile.y))
      .map((tile) => ({
        x: tile.x,
        y: tile.y,
      }));
  ...
  computeGameOutcomes(team: Team, otherTeam: Team, winnerTeamId: number): OutcomeData {
    try {
      ...
      return {
        ...
        key:
          this.mode == PlayMode.PRACTICE || this.mode == PlayMode.TUTORIAL ? null : (team.getChestKey() as ChestColor),
  ...
  computeAudienceRewards(score: number, chests: Array<GameOutcomeReward>): void {
    ...
    let rewards =
      this.mode == PlayMode.RANKED || this.mode == PlayMode.RANKED_VS_AI ? leagueRewards[this.league] : casualRewards;
  ...
  computeGrade(team: Team, otherTeam: Team) {
    ...
    return (
      (hpFactor * hpCoefficient +
        healingFactor * healingCoefficient +
        offenseFactor * offenseCoefficient +
        levelFactor * levelCoefficient) /
      totalWeight
    );
  }
  ...
  async incrementStartedGames(team: Team) {
    try {
      await apiFetch("incrementStartedGames", "", {
        method: "POST",
        body: {
  ...
  async saveInventoryToDb(token: string, characterId: string, inventory: number[]) {
    ...
    try {
      await apiFetch("inventorySave", token, {
        method: "POST",
        body: {
          characterId,
          inventory,
  ...
  async saveGameAction(playerId: string, action: GameAction, details: any) {
    try {
      await apiFetch("insertGameAction", "", {
        method: "POST",
        body: {
          gameId: this.id,
          playerId,
