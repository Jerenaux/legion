...
export class AIGame extends Game {
  ...
  addAICharacter(team: Team, character: DBCharacterData, position?: { x: number; y: number }) {
    ...
    const newCharacter = new AIServerPlayer(
  ...
  async fetchZombieData(elo: number) {
    try {
      ...
      const data = await apiFetch(`zombieData?league=${league}&elo=${elo}`, "", {
        headers: {
  ...
  async populateTeams() {
    ...
    this.turnDuration = Math.max(GAME_0_TURN_DURATION - playerTeam.getCompletedGames() * 20, TURN_DURATION);
    ...
    if (this.mode === PlayMode.CASUAL_VS_AI || this.mode === PlayMode.RANKED_VS_AI) {
      ...
      // Create a matching team for practice
      if (this.mode != PlayMode.TUTORIAL) {
        this.createAITeam(aiTeam!, nb, levels);
      }
    }
    ...
    if (AImodes.includes(this.mode)) {
      ...
      if (winRatio > 0.9) {
        if (!isVsZombie || (this.mode === PlayMode.RANKED_VS_AI && this.league >= League.GOLD))
          this.addExtraTeamMember(aiTeam!);
        aiTeam.scaleStats(1.2);
      }

      if (isVsZombie && winRatio >= 0.5) {
        const averageLevel = Math.ceil(
          aiTeam.getMembers().reduce((sum, player) => sum + player.level, 0) / aiTeam.getMembers().length,
        );
  ...
  addExtraTeamMember(team: Team) {
    ...
    const averageLevel = Math.ceil(
      team.getMembers().reduce((sum, player) => sum + player.level, 0) / team.getMembers().length,
    );
  ...
  putInFormation() {
    ...
    [Class.WARRIOR, Class.WHITE_MAGE, Class.BLACK_MAGE].forEach((className, index) => {
      ...
      this.summonEnemy({
        x: newPosition.x,
        y: newPosition.y,
        attackMode: AIAttackMode.UNLIMITED,
        className,
        full: true,
      });
    });
