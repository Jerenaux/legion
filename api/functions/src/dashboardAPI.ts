...
function adminOnRequest(optionsOrHandler: any, maybeHandler?: DashboardHandler): any {
  ...
  return onRequest({ ...optionsOrHandler, secrets: ["API_KEY"] }, adminHandler(maybeHandler!));
}
...
export async function logPlayerAction(playerId: string, actionType: string, details: any) {
  ...
  console.log(
    `[logPlayerAction] playerId: ${playerId}, actionType: ${actionType}, details: ${JSON.stringify(details)}`,
  );
  const db = admin.firestore();
...
export const getDashboardData = adminOnRequest({ memory: "512MiB" }, async (request, response) => {
  const db = admin.firestore();
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      const medianGameDuration =
        gameDurations.length > 0 ? gameDurations.sort((a, b) => a - b)[Math.floor(gameDurations.length / 2)] : 0;
...
export const getActionLog = adminOnRequest({ memory: "512MiB" }, async (request, response) => {
  const db = admin.firestore();
...
export const getGameLog = adminOnRequest(async (request, response) => {
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      const gameDoc = gameQuerySnapshot.docs[0];
      const snapshot = await gameDoc.ref.collection("actions").orderBy("timestamp", "asc").limit(100).get();
...
export const listPlayerIDs = adminOnRequest({ memory: "512MiB" }, async (request, response) => {
  const db = admin.firestore();
  corsMiddleware(request, response, async () => {
    // if (!checkAPIKey(request)) {
    //     response.status(401).send('Unauthorized');
...
export const getEngagementMetrics = adminOnRequest({ memory: "512MiB" }, async (request, response) => {
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      let newPlayersQuery = db.collection("players").where("joinDate", ">=", startDate);
      ...
      const newPlayers = newPlayersSnapshot.docs.filter((doc) => !doc.data().is_excluded);
      const activePlayers = activePlayersSnapshot.docs.filter((doc) => !doc.data().is_excluded);
      ...
      const visitsQuery = db.collection("dailyVisits").where(admin.firestore.FieldPath.documentId(), ">=", startDate);
      ...
      let playedGamesQuery = db.collection("players").where("joinDate", ">=", playedGamesCutoff);

      let playedMultipleGamesQuery = db.collection("players").where("joinDate", ">=", playedMultipleGamesCutoff);
      ...
      const playedGamesDocs = playedGamesSnapshot.docs.filter((doc) => !doc.data().is_excluded);
      const playedMultipleGamesDocs = playedMultipleGamesSnapshot.docs.filter((doc) => !doc.data().is_excluded);
      ...
      const metrics = {
        ...
        totalPlayers,
        landingPageCvRate,
        playedOneGameRate: playedGamesDocs.length > 0 ? (playedOneGame / playedGamesDocs.length) * 100 : 0,
        playedMultipleGamesRate:
          playedMultipleGamesDocs.length > 0 ? (playedMultipleGames / playedMultipleGamesDocs.length) * 100 : 0,
        gameCompletionRate: totalGamesStarted > 0 ? (totalCompletedGames / totalGamesStarted) * 100 : 0,
        totalFirstGameAbandonmentRate:
          playedGamesDocs.length > 0 ? (abandonedFirstGame / playedGamesDocs.length) * 100 : 0,
      };
...
export const getTutorialDropoffStats = adminOnRequest(async (request, response) => {
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      const playersSnapshot = await db.collection("players").where("joinDate", ">=", startDate).get();
      ...
      const averageStepIndex = stepOrder.length > 0 ? Math.round(weightedSum / totalPlayers) : -1;
      stats.averageLastStep = averageStepIndex >= 0 ? stepOrder[averageStepIndex] : "none";
    ...
    try {
      ...
      for (let i = 0; i < playerIds.length; i += tutorialBatchSize) {
        ...
        const completedTutorials = new Map(
          tutorialResults.map((result, index) => [batchPlayerIds[index], !result.empty]),
        );
        ...
        // Log progress
        console.log(
          `Processed ${Math.min(i + tutorialBatchSize, playerIds.length)} out of ${playerIds.length} players`,
        );
      }
...
export const migrateMetricsToStats = adminOnRequest(async (request, response) => {
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      const playersSnapshot = await db.collection("players").where("engagementMetrics", "!=", null).get();
...
export const getPlayerGameHistory = adminOnRequest({ memory: "512MiB" }, async (request, response) => {
  const db = admin.firestore();
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      for (const gameDoc of gamesSnapshot.docs) {
        ...
        const actionsSnapshot = await gameDoc.ref.collection("actions").orderBy("timestamp", "asc").get();
...
export const migrateMobileFlag = adminOnRequest({ memory: "512MiB" }, async (request, response) => {
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      const docsToUpdate = playersSnapshot.docs.filter(
        (doc) => !Object.prototype.hasOwnProperty.call(doc.data(), "isMobile"),
      );
  ...
  corsMiddleware(request, response, async () => {
    try {
      ...
      for (const playerDoc of playersSnapshot.docs) {
        ...
        const actionsSnapshot = await playerDoc.ref.collection("actions").orderBy("timestamp", "asc").get();
