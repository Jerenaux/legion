...
import {
  eloRangeIncreaseInterval,
  eloRangeStart,
  eloRangeStep,
  goldRewardInterval,
  goldReward,
  casualModeThresholdTime,
  maxWaitTimeForPractice,
  ALLOW_SWITCHEROO_RANKED,
...
class Lobby {
  ...
  addPlayer(socket: any): boolean {
    ...
    console.log(
      // @ts-ignore
      `[matchmaker:Lobby:addPlayer] creatorSocket: ${this.creatorSocket?.uid}, creatorId: ${this.creatorId}, isCreator: ${socket.uid === this.creatorId}, opponentSocket: ${this.opponentSocket?.uid}`,
    );
...
function switcherooCheck(player: QueuingPlayer) {
  if (
    (player.mode == PlayMode.CASUAL || (ALLOW_SWITCHEROO_RANKED && player.mode == PlayMode.RANKED)) &&
    ...
    const redirectionProbability = Math.min(
      1,
      waitTimeBeyondThreshold / (maxWaitTimeForPractice - casualModeThresholdTime),
    );
    ...
    for (let j = i + 1; j < queue.length; j++) {
      ...
      if (player1.mode == player2.mode && canBeMatched(player1, player2)) {
        ...
        if (player1.mode == player2.mode) {
          ...
          console.log(
            `[matchmaker:tryMatchPlayers] Incompatible ELOs: ${player1.elo} vs ${player2.elo}, ranges: ${player1.range} vs ${player2.range}`,
          );
        }
      }
    }
...
function canBeMatched(player1: QueuingPlayer, player2: QueuingPlayer): boolean {
  ...
  const isEloCompatible =
    Math.abs(player1.elo - player2.elo) <= player1.range && Math.abs(player1.elo - player2.elo) <= player2.range;
...
async function createGame(
  player1: Socket,
  player2?: Socket,
  mode: PlayMode = PlayMode.PRACTICE,
  ...
  try {
    ...
    if (player2) player2.nsp.to(player2.id).emit("matchFound", { gameId });
...
async function addToQueue(socket: any, mode: PlayMode) {
  try {
    const queuingData = await apiFetch("queuingData", socket.firebaseToken);
...
async function logQueuingActivity(playerId: string, actionType: string, details: any) {
  ...
  try {
    await apiFetch("logQueuingActivity", "", {
      method: "POST",
      body: {
        playerId,
        actionType,
...
export async function processJoinLobby(socket, data: { lobbyId: string }) {
  try {
    ...
    const lobbyDetails = await apiFetch(`getLobbyDetails?lobbyId=${data.lobbyId}`, socket.firebaseToken);
    ...
    if (!lobby) {
      ...
      const newLobby = new Lobby(data.lobbyId, lobbyDetails.creatorId, lobbyDetails.opponentId);
      ...
      socket.emit("lobbyJoined", {
        ...
        opponentName:
          lobbyDetails.type === "friend"
            ? socket.uid === lobbyDetails.creatorId
              ? lobbyDetails.opponentNickname
              : lobbyDetails.nickname
            : null,
      });
      return;
    }
    ...
    if (lobby.isFull()) {
      ...
      await createGame(lobby.creatorSocket, lobby.opponentSocket, mode, null);
...
function findLobbyBySocketId(socketId: string): Lobby | undefined {
  return Array.from(lobbies.values()).find(
    (lobby) => lobby.creatorSocket?.id === socketId || lobby.opponentSocket?.id === socketId,
  );
}
...
export async function processSendChallenge(socket: any, data: { opponentUID: string }) {
  try {
    ...
    const challengerData = await apiFetch(`getProfileData?playerId=${socket.uid}`, socket.firebaseToken);

    // Get opponent's data
    const opponentData = await apiFetch(`getProfileData?playerId=${data.opponentUID}`, socket.firebaseToken);

    // Create the friend lobby via API
    try {
      const response = await apiFetch("createLobby", socket.firebaseToken, {
        method: "POST",
        body: {
          type: "friend",
...
export async function processChallengeDeclined(
  socket,
  data: {
    challengerId: string;
    lobbyId: string;
  ...
  console.log(
    `[matchmaker:processChallengeDeclined] Challenger ${data.challengerId} declined challenge for lobby ${data.lobbyId}`,
  );
...
function declinePendingChallenge(uid: string) {
  ...
  if (pendingChallenge) {
    ...
    if (challengerSocket) {
      ...
      // @ts-ignore
      cancelLobby(challengerSocket.firebaseToken, lobbyId, "pending challenge declined").catch(() => {}); // Error already logged in cancelLobby
    }
...
async function cancelLobby(creatorToken: string, lobbyId: string, reason: string = "") {
  try {
    await apiFetch("cancelLobby", creatorToken, {
      method: "POST",
      body: {
        lobbyId,
      },
