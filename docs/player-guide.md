...
All external HTTP(S)/WebSocket traffic is blocked in this harness. It substitutes only authentication, player data, telemetry API responses, and the battle transport; the app routes, components, assets, and Phaser renderer are real. Sentry uses its real Electron SDK/preload and a loopback-only ingestion server. The test checks main/renderer errors, rejected promises, console errors, player feedback, release identity, and native renderer exits. The release webpack configuration does not import anything in `client/tools/guide`. This is a renderer/navigation/reporting smoke test, not a production matchmaking or authentication test.
