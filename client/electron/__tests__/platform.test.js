...
test("uses an Itch app key when launched by the Itch app", async () => {
  await expect(
    getPlatformAuth({ ITCHIO_API_KEY: "itch-key" }, () => {
...
test("can force a direct session for local development", async () => {
  await expect(
    getPlatformAuth({ USE_DIRECT_AUTH: "true" }, () => {
      throw new Error("unused");
    }),
  ).resolves.toBeNull();
});
...
test("uses Steam's native gamepad keyboard and controller type when available", async () => {
  ...
  await expect(showGamepadTextInput({ description: "Name", maxCharacters: 12, existingText: "A" })).resolves.toBe(
    "Legionary",
  );
