        ...
        {(playerContext) => (
          <div className="menu">
              ...
              <Link href={`/profile/${this.props.playerData?.uid}`} className="avatarContainerLink">
                <div className="avatarContainer">
                  {this.state.isLoading ? (
                    ...
                    <div
                      className="avatar"
                      style={{ backgroundImage: this.state.avatarUrl ? `url(${this.state.avatarUrl})` : "none" }}
                    ></div>
                  )}
            ...
            <div className="menuItems">
              <Link
                href="/play"
                onMouseOver={() => this.setState({ hovered: MenuItems.PLAY })}
                onMouseLeave={() => this.setState({ hovered: "" })}
              >
                <div className={`menuItemContainer ${currentPage(Routes.PLAY) ? "activeFlag" : ""}`}>
                  <img
                    className="menuItem"
                    src={this.state.hovered === MenuItems.PLAY ? playActiveIcon : playIcon}
                    alt="Play"
                  />
                </div>
              </Link>
              <Link
                href="/team"
                onMouseOver={() => this.setState({ hovered: MenuItems.TEAM })}
                onMouseLeave={() => this.setState({ hovered: "" })}
              >
                <div className={`menuItemContainer ${currentPage(Routes.TEAM) ? "activeFlag" : ""}`} data-team-page>
                  <img
                    className="menuItem"
                    src={this.state.hovered === MenuItems.TEAM ? teamActiveIcon : teamIcon}
                    alt="Team"
                  />
                </div>
              </Link>
              ...
              >
                <div
                  className={`menuItemContainer ${currentPage(Routes.SHOP) ? "activeFlag" : ""} ${!playerContext.canAccessFeature(LockedFeatures.CONSUMABLES_BATCH_1) ? "disabled" : ""}`}
                >
                  <img
                    className="menuItem"
                  ...
                  {!playerContext.canAccessFeature(LockedFeatures.CONSUMABLES_BATCH_1) && (
                    <img className="lock-overlay" src={lockIcon} alt="Locked" />
                  )}
                </div>
              </Link>
              <Link
              ...
              >
                <div
                  className={`menuItemContainer ${currentPage(Routes.RANK) ? "activeFlag" : ""} ${!playerContext.canAccessFeature(LockedFeatures.RANKED_MODE) ? "disabled" : ""}`}
                >
                  <img
                    className="menuItem"
            ...
            <div className="flexContainer" id="goldEloArea">
              <UserInfoBar
                icon="gold"
                label={`${this.state.isLoading ? "Loading..." : this.formatNumber(Math.round(this.props.playerData?.gold))}`}
              />
              {playerContext.canAccessFeature(LockedFeatures.RANKED_MODE) && (
                <UserInfoBar
                  icon="league"
