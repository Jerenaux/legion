...
class Overview extends Component<Props, State> {
  constructor(props: Props) {
    super(props);
    this.state = {
      previousHPs: props.members ? props.members.map((member) => member.hp) : [],
      blinking: props.members ? props.members.map(() => false) : [],
      selected: null,
    };
  }
  ...
  render({ members, position, isSpectator }: Props, { blinking }: State) {
    ...
    return (
      <div
        className={`overview ${this.props.isPlayerTeam && "overview_playerteam"} ${position === "right" && "overview_right"}`}
      >
        <PlayerInfo
          player={this.props.player}
          isPlayerTeam={this.props.isPlayerTeam}
          position={this.props.position}
          isSpectator={isSpectator}
          eventEmitter={this.props.eventEmitter}
        />
        <div className="member_container">
          <div className="team_label">{this.props.isPlayerTeam ? "Your team" : "Enemy team"}</div>
            ...
            return (
              ...
              >
                <div className="char_profile_container" style={charProfileStyle(memberIndex)}>
                  <div
                    className={`char_portrait ${position === "left" ? "flip" : "char_portrait_right"} ${member.hp > 0 ? "char_alive_animation" : ""}`}
                    style={portraitStyle}
                  />
                </div>
                <div
                  className={`char_stats ${position === "right" && "char_stats_right"}`}
                  style={charStatStyle(memberIndex)}
                >
                ...
                <div className={`char_statuses ${position === "right" && "char_statuses_right"}`}>
                  {Object.keys(member?.statuses).map((status: StatusEffect) => {
                    return (
                      member.statuses[status] !== 0 && (
                        <img key={`${memberIndex}-${status}`} src={statusIcons[status]} alt="" />
                      )
                    );
                  })}
                </div>
