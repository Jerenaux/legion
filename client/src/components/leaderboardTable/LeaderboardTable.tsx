  ...
  handleSort(column: string, index: number) {
    ...
    const sortedData = [...this.state.tableData].sort((a, b) => {
      ...
      if (typeof aValue === "string" && typeof bValue === "string") {
        ...
        return this.state.isAscending[index] ? aValue.localeCompare(bValue) : bValue.localeCompare(aValue);
      }

      // Handle numeric values
      return this.state.isAscending[index] ? Number(aValue) - Number(bValue) : Number(bValue) - Number(aValue);
    });
    ...
    return (
      <div className="rank-table-container">
        <table className="rank-table">
              ...
              {columns.map((column, i) => (
                <th key={i} onClick={() => this.handleSort(columnType[column], i)}>
                  <div>
                    ...
                    {i !== 1 && i < 6 && (
                      <img className="thead-sort-icon" src={arrowIcon} alt="" style={sortIconStyle(i)} />
                    )}
                  </div>
                </th>
              ))}
          ...
          <tbody>
            {this.props.data ? (
              this.state.tableData.map((item: LeaderboardRow, index: number) => (
                <tr
                  key={index}
                ...
                >
                  <td className="rank-row">
                    <div className="rank-row-number" style={rankRowNumberStyle(item.rank)}>
                      {item.rank}
                    </div>
        ...
        <div
          style={this.state.tableData.length === 0 ? { display: "block" } : { display: "none" }}
