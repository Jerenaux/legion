...
class QueuePage extends Component<QPageProps, QpageState> {
  ...
  validateMode() {
    const validModes = [PlayMode.PRACTICE, PlayMode.CASUAL, PlayMode.RANKED] as number[];
  ...
  handleConnectError = (error?: Error) => {
    ...
    this.setState({
      queueDataLoaded: false,
      statusMessage: sessionExpired
        ? "Your session expired. Restart Legion to reconnect."
  ...
  setupSocketListeners = (socket) => {
    ...
    socket.on("lobbyJoined", (data) => {
      this.setState({
        statusMessage:
          data.type === "friend" ? `Waiting for ${data.opponentName} to join…` : "Waiting for another player to join…",
        statusIsError: false,
        lobbyDetails: {
          type: data.type,
    ...
    <div className="queue-info lobby-mode">
      ...
      <Link href="/play" className="cancel-game-link">
        <div className="queue-detail-footer centered">
          ...
          <div className="queue-footer-text">{isLobbyMode ? "CANCEL GAME" : "BACK TO PLAY"}</div>
        </div>
      </Link>
    </div>
  );
            ...
            isLobbyMode ? (
              ...
              <div className="queue-info">
                ...
                <div className="queue-count">
                  ...
                  >
                    <div>
                      <div className="queue-count-number">{queueData.nbInQueue}</div>
                      <div className="queue-count-text">Queueing</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    {ENABLE_MM_TOGGLE && (
                      <div className="queue-detail-header">
                        ...
                        <div
                          className={
                            this.state.findState === "accurate" ? "queue-detail-btn active" : "queue-detail-btn"
                          }
                    ...
                    <div className="queue-detail-body">
                      ...
                      <div>
                        ...
                        <div>
                          ...
                          <div>
                            <span style={{ color: "coral" }}>{this.state.earnedGold}</span>
                          </div>
                        </div>
                      </div>
                      <div>
                        ...
                        <div>
                          ...
                          <div>
                            <span style={{ color: "deepskyblue" }}>
                              {this.state.queueData.estimatedWaitingTime === -1
                                ? "?"
                                : this.state.queueData.estimatedWaitingTime}
                            </span>
                            &nbsp;
                    ...
                    <Link href="/play">
                      <div className="queue-detail-footer">
                        ...
                        <div className="queue-footer-text">LEAVE QUEUE</div>
                      </div>
                    </Link>
          ...
          <div className="queue-news">
            {newsLoaded ? (
              [...news].reverse().map((newsItem) => (
                <div className="queue-news-container" key={newsItem.title}>
                  <div className="queue-news-title">
                    <div>
                      <span style={{ color: "cyan" }}>{newsItem.title}</span>
                    </div>
                    <div className="queue-news-date">
                      <span style={{ color: "coral" }}>{newsItem.date}</span>
                    </div>
                  </div>
                  <div className="queue-news-content">{newsItem.text}</div>
              ...
              <div className="queue-news-loading">
                <Skeleton height={100} width={300} count={1} highlightColor="#0000004d" baseColor="#0f1421" />
              </div>
            )}
          </div>
        )}
