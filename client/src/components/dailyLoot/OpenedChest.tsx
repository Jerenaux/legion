...
class OpenedChest extends Component<OpenedChestProps> {
    ...
    return this.props.content.map((reward: ChestReward, idx: number) => {
      ...
      return (
        <div key={idx} className="streak_gold_list">
          ...
          ></div>
          <div className="streak_gold_list_amount">{reward.amount}</div>
        </div>
      );
    });
  }
  ...
  render() {
    return (
      <div
        style={{ top: `${document.documentElement.scrollTop}px`, overflow: "hidden", zIndex: "9999999999" }}
        className="light_streak_container"
      >
        <div className="light_streak" style={{ width: this.props.width * 0.5 }}>
          {this.props.content && <Confetti width={this.props.width * 0.5} height={this.props.height} />}
          ...
          <div className="streak_gold_list_container">{this.renderRewards()}</div>
          <div
            className="streak_cofirm_container"
            style={{ width: this.props.width * 0.8 }}
            onClick={this.props.onClick}
          >
            <div className="streak_confirm_btn">
              <span>Confirm</span>
            </div>
          </div>
        </div>
