...
class CharacterSheet extends Component<CharacterSheetProps> {
  ...
  handleOpenModal = (
    e: any,
    modalData: BaseItem | BaseSpell | BaseEquipment | SPSPendingData,
  ...
  handleUnEquipItem = (
    e: any,
    modalData: BaseItem | BaseSpell | BaseEquipment,
    modalType: ItemDialogType,
    index: number,
  ) => {
    if (!modalData) return;
  ...
  render() {
        ...
        <div data-sp-plus="true" className="character-info-bar" key={index}>
          ...
          <div className="curr-info-container">
            <p className="curr-info">
              <span
                style={
                  effectVal(item.key) > 0 ? { color: "#9ed94c" } : effectVal(item.key) < 0 ? { color: "#c95a74" } : {}
                }
              >
    ...
    const renderEquipmentItems = (itemCategory) => {
      ...
      return items.map((item, index) => {
        ...
        const slotStyle = {
          backgroundImage:
            item.value >= 0 && `linear-gradient(to bottom right, ${RarityColor[equipmentItem?.rarity]}, #1c1f25)`,
        };

        if (this.props.selectedEquipmentSlot > -1) {
          // Equipping something
          if (
            this.props.selectedEquipmentSlot == EquipmentSlot.LEFT_RING &&
          ...
          <div
            ...
            className={`sheet-item ${
              itemData !== undefined
                ? ""
                : (
                      this.props.selectedEquipmentSlot > -1 &&
                        equipmentSlotFields[this.props.selectedEquipmentSlot] === item.key
                    )
                  ? "blinking-gradient"
                  : ""
            }`}
        ...
        if (i < items.length) {
          ...
          return (
            <div
              ...
              key={i}
              style={
                (inventoryType === InventoryType.SPELLS || inventoryType === InventoryType.CONSUMABLES) && slotStyle
              }
      ...
      <div className="team-content-card-container">
        <div className="team-content-container">
          ...
          <div className="team-info-container">
            <div className="team-info">
              ...
              <div className="team-exp-info">
                <span>
                  EXP <span className="team-curr-exp">{Math.round(characterData?.xp)}</span> /{" "}
                  <span className="team-total-exp">{xpToLevel}</span>
                </span>
              </div>
            </div>
          ...
          <div className="team-character-info-container">
            ...
            <div className="team-character-info">{renderInfoBars()}</div>
          </div>
          <div className="team-items-container">
            <div className="character-icon-container">{renderEquipmentItems("specialEquip")}</div>
            {characterData.skill_slots > 0 && (
              <div className="team-item-container">
                <p className="team-item-heading">SPELLS</p>
                <div className="team-items">{renderInventoryItems(InventoryType.SPELLS)}</div>
              </div>
            )}
            <div className="team-item-container">
              <p className="team-item-heading">ITEMS</p>
              <div className="team-items">{renderInventoryItems(InventoryType.CONSUMABLES)}</div>
            </div>
          </div>
        </div>
        <div className="team-equip-container">{renderEquipmentItems("standardEquip")}</div>
        <ItemDialog
          isEquipped={true}
