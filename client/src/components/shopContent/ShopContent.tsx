  ...
  render() {
    ...
    const renderItems = () => {
      switch (this.state.curr_tab) {
        case ShopTab.SPELLS: {
          const unlockedSpells = this.state.inventoryData.spells.filter((spell) =>
            this.context.canAccessFeature(spell.unlock),
          );
            ...
            <div className="spells-container">
              {Object.entries(groupedSpells).map(([category, spells]) => (
                <div key={category} className="spells-section">
                  <h3 className="spells-type-title">{category.charAt(0) + category.slice(1).toLowerCase()}</h3>
                  <div className="spells-grid">
                    {spells.map((spell, index) => (
                      <ShopSpellCard
                        ...
                        isHighlighted={
                          this.props.highlightedItemId !== undefined &&
                          spell.id === Number(this.props.highlightedItemId)
                        }
                      />
                    ))}
                  </div>
        ...
        case ShopTab.CONSUMABLES: {
          const unlockedConsumables = this.state.inventoryData.consumables.filter((consumable) =>
            this.context.canAccessFeature(consumable.unlock),
          );
            ...
            <div className="consumables-container">
              {Object.entries(groupedConsumables).map(([category, items]) => (
                <div key={category} className="consumables-section">
                  <h3 className="consumables-type-title">{category.charAt(0) + category.slice(1).toLowerCase()}</h3>
                  <div className="consumables-grid">
                    {items.map((item, index) => (
                      <ShopConsumableCard
                        ...
                        handleOpenModal={this.handleOpenModal}
                        isHighlighted={
                          this.props.highlightedItemId !== undefined && item.id === Number(this.props.highlightedItemId)
                        }
                      />
                    ))}
                  </div>
              ...
              {Object.entries(groupedEquipment).map(([category, items]) => (
                <div key={category} className="equipment-section">
                  <h3 className="equipment-type-title">
                    {useDetailedCategories ? equipmentSlotLabelsPlural[category as unknown as EquipmentSlot] : category}
                  </h3>
    ...
    const renderSkeletons = () => (
      ...
      >
        {[...Array(12)].map((_, index) => (
          <div key={index}>
            <Skeleton height={320} width={200} highlightColor="#0000004d" baseColor="#0f1421" />
          </div>
        ))}
      </div>
    );
        ...
        <div className="shop-tabs-container">
          {this.state.inventoryData &&
            shopTabIcons.map((icon, index) => {
              ...
              const isDisabled =
                (isCharacterTab && !this.context.canAccessFeature(LockedFeatures.CHARACTER_PURCHASES)) ||
