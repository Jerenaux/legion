...
class App extends Component<{}, AppState> {
  ...
  handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (!action || (typing && action !== "cancel") || (event.code === "Tab" && this.state.currentMainRoute !== "game"))
      return;
  ...
  handleDesktopAction = async (event: CustomEvent<{ action: DesktopAction; source: string }>) => {
    ...
    if (action === "cancel") {
      const cancel = Array.from(document.querySelectorAll<HTMLElement>("[data-desktop-cancel]")).find(
  ...
  handleRoute = (
    ...
    refreshAllData: () => void,
    updateActiveCharacter: (id: string | null) => void,
  ) => {
  ...
  shouldComponentUpdate(nextProps: {}, nextState: AppState) {
    return this.state.currentUrl !== nextState.currentUrl || this.state.currentMainRoute !== nextState.currentMainRoute;
  }
