...
export class Player extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    if (isPlayer) {
      this.numKey = scene.add
        .text(30, 70, num.toString(), {
          fontFamily: "Kim",
          fontSize: "12px",
          color: "#fff",
          stroke: "#000",
  ...
  setUpStatusEffects() {
    ...
    statusEffects.forEach((effect) => {
      ...
      const sprite = this.scene.add
        .sprite(0 + xOffset, -20 + yOffset, "statuses")
        .setOrigin(0.5, 0.1)
        .setVisible(false);
  ...
  makeAirEntrance() {
    ...
    const createTrail = () => {
      const trailSprite = this.scene.add
        .sprite(this.x, this.y, this.texture)
        .setAlpha(0.5)
    ...
    this.scene.tweens.add({
      ...
      onComplete: () => {
        ...
        const smokeSprite = this.scene.add
          .sprite(targetX, targetY + 15, "")
  ...
  select() {
    ...
    if (this.isPlayer) {
      ...
      const cheapestSpell = this.spells.reduce(
        (cheapest, spell) => (spell.cost < cheapest.cost ? spell : cheapest),
        this.spells[0],
      );
  ...
  castAnimation(flag: boolean, name: string, charge?: string) {
    if (flag) {
      ...
      if (charge) {
        ...
        this.chargeSprite = this.scene.add
          .sprite(
            0, // Center on player
  ...
  isTarget() {
    return (!this.isPlayer || this.isInIce()) && this.isAlive();
  ...
  displayOverheadText(text, duration, color) {
    ...
    const textObject = this.scene.add
      .text(randomXOffset, -this.sprite.height / 2 + 15 + randomYOffset, `${String(text)}`, {
        fontSize: "24px",
        color,
        stroke: "#000",
        strokeThickness: 3,
  ...
  checkDeathAnimation() {
    if (
      !this.isAlive() &&
      (!this.sprite.anims.isPlaying || this.sprite.anims.currentAnim?.key !== `${this.texture}_anim_die`)
    ) {
      this.playAnim("die");
    }
  }
