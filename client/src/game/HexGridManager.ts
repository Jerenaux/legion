...
export class HexGridManager {
  ...
  floatHexTiles(
    duration: number,
    onTileClickHandler: (x: number, y: number) => void,
    onTileHoverHandler: (x: number, y: number, hover?: boolean) => void,
  ) {
    const { startX, startY } = this.gridCorners;
    const offsetX = startX + HexGridManager.HEX_WIDTH / 2;
    const offsetY = startY + HexGridManager.HEX_HEIGHT / 2;
  ...
  private floatOneHexTile(
    ...
    this.tileStates.set(serializeCoords(x, y), {
      baseColor: TileColors.NORMAL,
      highlights: new Map(),
    });
  ...
  isValidGridPosition(x: number, y: number): boolean {
    return x >= 0 && x < GRID_WIDTH && y >= 0 && y < GRID_HEIGHT && !isSkip(x, y) && !this.isHole(x, y);
  }

  isValidCell(fromX: number, fromY: number, toX: number, toY: number, isFree: (x: number, y: number) => boolean) {
    return this.isValidGridPosition(toX, toY) && isFree(toX, toY) && lineOfSight(fromX, fromY, toX, toY, isFree);
  }

  // Highlight method for tiles in a radius
  highlightTilesInRadius(
    gridX: number,
    gridY: number,
    radius: number,
    color: number = TileColors.MOVEMENT_RANGE,
    shouldHighlight?: (x: number, y: number) => boolean,
    type: HighlightType = HighlightType.MOVEMENT,
  ) {
  ...
  highlightSpellRadius(
    gridX: number,
    gridY: number,
    radius: number,
    gridMap: Map<string, Player>,
    isAllyCallback: (player: Player) => boolean,
    isAllyTargetingSpell: boolean = false,
  ) {
    const tilesInRadius = this.getTilesInRadius(gridX, gridY, radius);
