...
class PlayerProvider extends Component<{}, PlayerContextState> {
  ...
  fetchFriends = async () => {
    ...
    try {
      ...
      const friends = await apiFetch(`listFriends?playerId=${user.uid}`, {}, 3);
  ...
  setupSocket = async () => {
    ...
    socket.on(
      "challengeReceived",
      (data: { challengerId: string; challengerName: string; challengerAvatar: string; lobbyId: string }) => {
  ...
  hasEquipableEquipmentByCurrentCharacter = (): boolean => {
    return this.state.player.inventory.equipment.some((equipment) =>
      canEquipEquipment(this.getActiveCharacter(), equipment),
    );
  };
  ...
  getEquipmentThatCurrentCharacterCanEquip = (): number => {
    return this.state.player.inventory.equipment.find((equipment) =>
      canEquipEquipment(this.getActiveCharacter(), equipment),
    );
  };
          ...
          <div className="modal-overlay">
            <div className="modal challenge-modal">
              <div
                ...
                style={{
                  backgroundImage: `url(${avatarContext(`./${this.state.challengeModal.challengerAvatar}.png`)})`,
                }}
              />
              <h3>Duel</h3>
              <div className="challenge-description">
                <p>
                  <span className="highlight-name">{this.state.challengeModal.challengerName}</span> has challenged you
                  to a duel!
                </p>
              </div>
              <div className="modal-footer">
                <button onClick={this.handleChallengeDecline} className="cancel-btn">
                  Decline
                </button>
                <button onClick={this.handleChallengeAccept} className="confirm-btn">
                  Accept
                </button>
              </div>
            </div>
