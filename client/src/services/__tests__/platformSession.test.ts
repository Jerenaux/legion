...
import { exchangePlatformCredential, getOrCreateDeviceId, getPlatformCredential } from "../platformSession";
...
describe("desktop platform sessions", () => {
  test("prefers a native Steam or Itch credential", async () => {
    ...
    await expect(
      getPlatformCredential({ getPlatformAuth: async () => native }, storage, () => "unused"),
  ...
  test("persists one direct-download device identity", () => {
    ...
    expect(getOrCreateDeviceId(adapter, () => "12345678-1234-4123-8123-123456789abc")).toBe(
      "12345678-1234-4123-8123-123456789abc",
    );
    expect(getOrCreateDeviceId(adapter, () => "different-device-id-123456789")).toBe(
      "12345678-1234-4123-8123-123456789abc",
    );
  });
  ...
  test("exchanges the credential for a Firebase custom token", async () => {
    ...
    await expect(
      exchangePlatformCredential("https://api", { provider: "itch", credential: "key" }, fetcher),
